...
MODEL_XML_PATH = os.path.join(fr"D:\Research\ICCSSE\ParalllelCableDrivenRobot\SpaceRobotEnv\assets", "ParallelCableDrivenRobot_v0.xml")


class ParallelCableDrivenRobot_v0(gym.Env):
    ...
    def reset_model(self):
        ...
        # v0-run6[0,0,0,0,0,-20]
        self.sim.model.body_pos[self.sim.model.body_name2id('object_point')] = np.array([1.78, 0, 0.1])
        
        # v0-run7[0,-10,0,-10,0,-10]
        # self.sim.model.body_pos[self.sim.model.body_name2id('object_point')] = np.array([1.72960777,0.,0.35626733])
        
        # v0-run8[0,-5,0,-5,0,-5]
        # self.sim.model.body_pos[self.sim.model.body_name2id('object_point')] = np.array([1.78,0,0.18])
        self.sim.forward()
    ...
    def _get_reward(self, current_info, action):
        ...
        ## 0
        # reward = - distance \
        #          - np.log(distance + 1e-6) \
        #          - 0.1 * (np.linalg.norm(end_effector_velocity) + np.linalg.norm(base_velocity)) \
        #          - 0.1 * (np.linalg.norm(base_position))

        # # 1
        # reward = - 5 * distance \
        #          - np.log(distance + 1e-6) \
        #          - 0.1 * (np.linalg.norm(end_effector_velocity) + np.linalg.norm(base_velocity)) \
        #          - 0.1 * (np.linalg.norm(action) + np.linalg.norm(base_position))
        ...
        # reward of 0225_3
        # reward of 0511
        # reward = - 5*distance - 2*np.log(distance + 1e-6) \
                # - 0.01 * (np.linalg.norm(end_effector_velocity)+ np.linalg.norm(base_velocity))
        # reward of 0512
        reward = - distance - np.log(distance + 1e-6)
    ...
    def _get_terminal(self, current_info):
        return not current_info["distance"] >= 0.05 # 0.05
